...
- [ ] 把比赛控制面彻底统一成 `openai-agents` 的 MCP 接口
说明：
当前仍然保留了手写的 [contest_mcp.py](/Users/nonoge/Desktop/auto_pentest/niuniu-agent/src/niuniu_agent/contest_mcp.py)。
目标是后续把这层降到最薄，或者完全移除，统一到 OpenAI Agents MCP 语义。

- [ ] 把四赛道能力改成“通用 skills + 路由组合”
说明：
当前已经有基础的控制平面和 loop，但通用能力模块还没有完整落成。

- [ ] 把 prompt 收敛成“一个入口 prompt + 几个关键触发 prompt”
说明：
目前 prompt 已经开始收口，但还没有形成完整的 trigger prompt 体系。
...
- [ ] 启动时检查工具是否存在
- [ ] 缺失工具时给出明确日志
- [ ] 缺失工具时自动退回到 Python / curl 替代路径
- [ ] 在 `debug` 模式中支持显式查看工具可用性
...
- [ ] 赛道不要直接绑定单个 skill，而是绑定 skill 组合
- [ ] 优先从 `recon_*` 开始，再进入 `exploit_*`
- [ ] 拿到 foothold 后再触发 `pivot_lateral` / `privesc_maintain`
- [ ] 任何疑似 flag 都交给 `flag_submit_recovery`
...
- [ ] 入口 prompt 根据题目描述先做第一轮技能选择
- [ ] 侦察结束 trigger prompt 重新评估下一技能
- [ ] 利用失败 trigger prompt 切换技能或回退到侦察
...
- [ ] 用“能力模块”替代“赛道专属硬编码技能”
- [ ] 实现赛道到技能组合的路由策略
- [ ] 为每个 skill 增加可复用的描述、触发条件、输入输出约束
...
- [ ] 识别并删除已经不再作为主路径的旧模块
- [ ] 清理旧的 controller/debug_chat/llm 兼容残留
- [ ] 清理旧测试或将其迁移到新架构
- [ ] 让 README 只描述新架构，不再混用旧术语

## 当前我建议的优先级

1. 统一 MCP 接口层
2. 落地通用 skills 注册表
3. 落地入口 prompt + trigger prompts
4. 做强 `competition` 不停机恢复
5. 再清理旧实现残留

## 当前最关键的风险

- [ ] 当前架构虽然已经能交互，但四赛道能力模块还不够完整
- [ ] `competition` 模式的“永不停止”语义还需要更强的恢复策略
- [ ] 旧实现残留模块仍然存在，后续需要继续收敛
