...
class ModelProviderRouter:
    ...
    async def create_completion(self, **kwargs: Any) -> Any:
        ...
        for provider in self.execution_order():
            ...
            request_kwargs = self._prepare_chat_kwargs_for_provider(provider, request_kwargs)
    ...
    @staticmethod
    def _prepare_chat_kwargs_for_provider(
        provider: ModelProviderConfig,
        kwargs: dict[str, Any],
    ) -> dict[str, Any]:
        prepared = dict(kwargs)
        messages = [dict(message) for message in list(prepared.get("messages") or [])]
        if provider.provider_id == "rightcodes" and messages and messages[0].get("role") == "system":
