...
ENTRY_PROMPT = TriggerPrompt(
    ...
    body=(
        ...
        " Do not require a reverse shell or callback by default. A reverse shell is optional, not a goal in itself."
        " Only use a reverse shell, reverse tunnel, or callback path after direct webshell execution, forward probing, local file-read, SSRF, or uploaded lightweight proxy tooling cannot answer the next step."
...
CHALLENGE_TAKEOVER_PROMPT = TriggerPrompt(
    ...
    body=(
        ...
        "Do not treat reverse shells or callback tunnels as required milestones. Prefer direct exploitation, file-read, webshell execution, SSRF, and forward pivots first; only fall back to reverse callback paths when the live evidence shows they are necessary. "
